...
class EnigmaGUI(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # Create a QTimer object to reset button stylesi
        ...
        self.mixLettersMap = {}
        self.lastColor = 0
        self.colors = ['Red','Blue','Green','Yellow','DeepPink','Orange','Brown','Lime','LightSalmon','Gray','Pink','Teal','SlateBlue','Light Brown']
        self.buttonsColors = {}
        ...
        if text and count > 0 and text[len(text) - 1] in 'abcdefghijklmnopqrstuvwxyz ':
            if count > 1:
                ...
                if text[len(text) - 1] != ' ':
                    ...
                    self.output_output.setText(self.outputText)
                self.spinTheWheels(1)
        if count < 0 and self.prevText[-1] in 'abcdefghijklmnopqrstuvwxyz ':
    ...
    def spinTheWheels(self, count):
        ...
        #     self.rotor2_input.setValue((((int(self.rotor3_input.text()) + count  ) // 26) + int(self.rotor2_input.text())) % 26)
        #     self.rotor1_input.setValue((int(self.rotor1_input.text()) + ((int(self.rotor2_input.text()) + count) // 26) )% 26)
    ...
    def resetKeyboardButtonStyle(self):
        ...
        if len(self.outputText) > 0:
            lastOutput = self.last_keyboard_button.text()  #self.outputText[len(self.outputText)-1]
